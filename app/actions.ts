...
export async function updateMemberProfileAction(memberId: string, formData: FormData) { if (!await requireMemberAdmin()) return; const name = formData.get("name")?.toString().trim(); const phone = formData.get("phone")?.toString().trim(); const email = formData.get("email")?.toString().trim().toLowerCase(); const pledgeClass = formData.get("pledgeClass")?.toString().trim(); if (!name || !phone || !email || !/^\S+@\S+\.\S+$/.test(email)) return; const member = await prisma.user.findUnique({ where: { id: memberId } }); if (!member) return; await prisma.user.update({ where: { id: memberId }, data: { name, phone, email, pledgeClass: pledgeClass || null } }); if (member.name !== name) { await prisma.dateAssignment.updateMany({ where: { memberOne: member.name }, data: { memberOne: name } }); await prisma.dateAssignment.updateMany({ where: { memberTwo: member.name }, data: { memberTwo: name } }); } revalidatePath("/profile-management"); revalidatePath("/your-profile"); }
...
export async function markDateFeedbackCompleteAction(assignmentId: string) { const userId = await getUserSessionId(); if (!userId) return; await prisma.dateFeedbackCompletion.upsert({ where: { assignmentId_userId: { assignmentId, userId } }, update: { completedAt: new Date() }, create: { assignmentId, userId } }); revalidatePath("/your-profile"); }
